    //Filtering region dropdown #1 첫번째 필터 - 필터가 change되어 select 되면 함수 실행 (Regieon)
    ...
    //Filtering role dropdown #2 두번째 필터 함수(Role)
    //#roleDropdown : 2번째 셀릭트 태그 id값
    $('body').on("change", '#roleDropdown', function() {
        var filter, table, tr, td, i;
        //
        var regionval = $('#regionDropdown :selected').val();  //첫번째 필터에서 선택한 값이 있으면 가져와서 저장 (기본값 all)
        //console.log(regionval, "regionval");
        filter = $(this).val(); //두번째 필터의 선택 값에 해당하는 td 데이터 가져오기
    ...
    //Show active inactive users #3 세번째 필터링 함수 - UserActive 여부
    //'#associateStatusDropdown' : 3번째 셀렉트 태그 id 값
    $('body').on("change", '#associateStatusDropdown', function() { //change 될 때 실행되고, 
                var filter, table, tr, td, i;
                filter = $(this).val(); // 세번째 필터에서 선택한 값에 해당하는 td의 값 (yes/no)
                ...
                var regionval = $('#regionDropdown :selected').val(); //첫번째 필터의 선택값 가져옴
                var roleval = $('#roleDropdown :selected').val(); //두번째 필터의 선택값 가져옴
